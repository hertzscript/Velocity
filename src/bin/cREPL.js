...
(function () {
	...
	const fs = require("fs");
	const os = require("os");
	// Runs all compiled source code concurrently in the event loop
	...
	// Import previous session(s) input history
	var prevHistory = [];
	if (writeHistory) {
	...
	// Writes session input history and terminates the application
	function exit() {
	...
	// Set to true if user sent an exit signal as their last action,
...
Press CTRL+P or type .pause to pause/unpause coroutine execution.`);
	var paused = false;
	// Pauses dispatcher execution
	function pauseExec() {
		logText("Coroutine execution paused.");
		paused = true;
		drawStats();
	}
	// Kills all coroutines
	function resetQueue() {
		while (hzDisp.queue.blocks.length > 0) hzDisp.killLast();
		logText("All coroutines terminated.");
	}
	// Starts dispatcher execution
	function startExec() {
		logText("Coroutine execution started.")
		paused = false;
		hzDisp.running = true;
	...
	function dotcodeHandler(input) {
		// Exit application
		if (input === ".exit") exit();
		// Pause/unpause all dispatcher execution
		else if (input === ".pause") paused ? startExec() : pauseExec();
		// Terminates all coroutines
		else if (input === ".kill") resetQueue();
		else return false;
		return true;
	}
	...
	const inputHandler = (error, input) => {
		...
		if (input[0] !== "." || !dotcodeHandler(input)) {
			...
			try {
				...
				hzDisp.import(generateHzModule(hzCompile("(function (){" + input + "})", false, false, true)));
